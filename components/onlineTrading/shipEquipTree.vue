...
<script>
  ...
  export default {
    name: "productList",
    props:['width','ifPerson','id','ifAddShip','type'], //  type:'0'  1个人中心里的  2设计方案里的 3装备在线船舶详情里的
    ...
    methods:{
      ...
      async toCheck(m,n){
          ...
          if(this.list[m].equipList[n].check==true){
              this.list[m].equipList[n].check=false
          }else{
        ...
        if(this.type==1){ //个人中心
          console.log("个人中心。。。"+this.list[m].equipList[n].id.toString())
            //如果选中，则调用这级分类的id，否则调用父级分类的id
            if(this.list[m].equipList[n].check){
                this.query.toW(qry,'cids',this.list[m].equipList[n].id.toString(),'LK')
            }else{
                this.query.toW(qry,'cids',this.list[m].id.toString(),'LK')
            }
...
<style>
  ...
  .pageList .table .el-table__expand-icon{
    margin-right: 0 !important;
  }
   .pageList .table .el-table__body-wrapper{
	   overflow-y: auto;
   }
  .pageList .table tr{
    cursor: pointer;
