    ...
    computed: {
      ...
      outZoomerPosition() {
        return {
          ...
          right:`${(this.imgW - this.imgWidth)/2-8}px`
        };
      },
      /**
       * 高清图地址地址
        ...
        if (!this.disabledReactive) {
          ...
          this.beforeReactivateMoveFns.push(() => {
            ...
            const rect = {
              ...
              right: dom.getBoundingClientRect().right,
      ...
      loadImg(url) {
        return new Promise((resolve, reject) => {
          ...
          myImg.onload = ()=>{
            ...
            let size = this.imgWidth/this.imgHeight
            // console.log(myImg.width/myImg.height>size)
            if(myImg.width/myImg.height>size){
              this.imgW = this.imgWidth
              this.imgH = this.imgWidth /( myImg.width/myImg.height)
            }
            if(myImg.width/myImg.height<=size){
              this.imgH = this.imgHeight
              this.imgW =this.imgHeight * (myImg.width/myImg.height)
            }
