...
<!--      <p :class="{active:item.isActive}" @click="toPage(item.href)" v-for="(item,index) in navList[0]" :key="index">{{item.nm}}</p>-->
      <p :class="{active:cd && item.cd == cd}" @click="toPage(item.href)"><img :src="item.img" alt=""/>{{item.nm}}</p>
...
<script>
  ...
  import sO from "../../assets/img/personal/销售订单.png"
  import mP from "../../assets/img/personal/我的产品.png"
  import wM from "../../assets/img/personal/往来款项.png"
  ...
  import dp from "../../assets/img/personal/店铺.png"
          ...
          if(this.userInfo.pid){  //有pid就是子账号
            ...
            this.navList.forEach((item,index)=>{
              if(item.title && item.nm!='我的资料'){
                item.title.forEach(i=>{
                  if(!i.flag && url.indexOf('http')==-1){
                    i.href=''
                  }
                })
              }else{
                if(!item.flag && url.indexOf('http')==-1 && item.nm!='快速下单'){
                  item.href=''
                }
              }
              this.$set(this.navList,index,this.navList[index])
