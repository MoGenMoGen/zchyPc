...
<!--      <p :class="{active:item.isActive}" @click="toPage(item.href)" v-for="(item,index) in navList[0]" :key="index">{{item.nm}}</p>-->
      ...
      <p v-for="(item2,index2) in item.title" :key="index2" :style="{'margin-left':item2.marginL+'px'}" :class="{active:item2.cd == cd}" @click="toPage(item2.href)">
...
<script>
  ...
  import cS from "../../assets/img/personal/船舶管控.png"
