...
<!--    船舶信息-->
      ...
      <div class="content">
        <div class="left">
          <div>
            <p class="left-title">报告名称：</p>
            <p class="left-msg">
              <input type="text" v-model="form.nm" placeholder="请输入报告名称">
    ...
    <div class="certificate">
      <div class="title">
        ...
        <p  @click="currentType=0" :class="{active:currentType===0}">审核不通过<span>({{listNo.length}})</span></p>
      </div>
      <!--通过审核-->
    ...
    export default {
        ...
        methods:{
          ...
          toSubmit(){
            ...
            this.form.validUntil=timeC.year+'-'+timeC.month+'-'+timeC.day+' '+timeC.hour+':'+timeC.minite+':'+timeC.second
            console.log(this.form)
            if(!this.form.nm){
              this.$message({
                message: '请输入名称',
          ...
          async getInfo(){
            ...
            let data=await this.api.jianyanList(this.query.toEncode(qry))
            if(data.length>0){
          ...
          async getInfoNo(){
            ...
            this.query.toW(qry,'docsId',this.id,'EQ')
            this.query.toP(qry,'1','999')
            let data=await this.api.jianyanListNo(this.query.toEncode(qry))
            if(data.length>0){
              data.forEach((item,index)=>{
                item.validUntil=item.validUntil?item.validUntil.split(' ')[0]:''
                item.imgList=item.imgUrl?item.imgUrl.split(','):[]
