      ...
      <div class="body">
        <div class="mainView">
          <div class="ship" v-for="(item,index) in shipList" :key="index" @click="toProDetail(item)">
            ...
            <div class="left2">
              <p v-html="item.nm"></p>
              <img v-if="search.used == 1" :src="tp1" alt=""/><img v-else :src="tp2" alt=""/>
            </div>
            <!--中部船舶属性-->
            <div class="mid">
              <p><span>船舶总长：</span>{{item.tl}}</p>
              <p><span>公约船长：</span>{{item.ml}}</p>
              <p><span>型宽：</span>{{item.wd}}</p>
              <p><span>型深：</span>{{item.hd}}</p>
            </div>
            <!--右一价格-->
            <div class="right1">
    ...
    methods:{
      async getData(){
        if(this.search.keywords){
          ...
          this.shipList = data.data.result
