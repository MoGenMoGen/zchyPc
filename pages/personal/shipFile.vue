         ...
         <el-table-column
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <!--船厂、设计院、监理监造、检验检测-->
              <el-button type="text" size="small" class="fileBtn" v-if="currentRole.identityCd=='identity30' || currentRole.identityCd=='identity20' || currentRole.identityCd=='identity50' || currentRole.identityCd=='identity40'"  @click="toNotice(scope.row)">节点通知</el-button>
            </template>
          </el-table-column>
       </el-table>
     </div>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        toDetail(item){
          console.log(item.id)
          this.$router.push('./shipFileDetail?id='+item.id+"&cdType=2&formW=design&shipCd="+item.cd+"&shipStatus="+item.statusNm)
        },
        back(){//返回
          this.$router.back()
        },
