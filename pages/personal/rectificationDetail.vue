  ...
  <div class="main">
    ...
    <div id="Issueshow" v-if="Issueshow">
      <div class="addBox">
        <div class="head">
          <p>再次下发说明</p>
          <img src="~@/assets/img/close.png" @click="Issueshow = false" class="point" />
        </div>
        <div class="addBody">
          <div style="display: flex; margin: 30px 0">
            ...
            <el-input type="textarea" :rows="8" placeholder="请输入二次下发内容" v-model="info.reissueReport"
              style="width: 327px"></el-input>
          </div>
          ...
          <div style="display: flex">
            ...
            <el-upload style="max-width: 327px" multiple ref="upload" action="/general/oss/upload"
              accept="image/png,image/gif,image/jpg,image/jpeg" list-type="picture-card" :auto-upload="true"
              :before-upload="handleBeforeUpload" :on-preview="handlePictureCardPreview" :on-remove="handleRemove"
    ...
    <div class="content">
      ...
      <h3 style="padding: 16px 0px 10px 0;margin-left: 20px;border-bottom: 1px solid rgba(0,0,0,0.1);font-size: 15px;">
        下发内容</h3>
      ...
      <div class="conwrapper">
        <div style="display: flex">
          ...
          <div class="righttextpart">
            ...
            <div class="itemstyle" v-if="info.rectifyerSign && info.rectifyerSign.includes('http')">
              整改责任人：
              <img :src="info.rectifyerSign" alt="" style="object-fit: fill; width: 60px; height: 40px" v-viewer />
            </div>
        ...
        <div class="lefttextpart">
          <div class="itemstyle" v-if="info.reviewerSign && info.reviewerSign.includes('http')">
            复查人：
            <img :src="info.reviewerSign" alt="" style="object-fit: fill; width: 60px; height: 40px" v-viewer />
          </div>
      ...
      <div v-if="info.rectifyReport" style="padding-bottom: 10px">
        <h3
          style="padding: 16px 0px 10px 0;margin-left: 20px;border-bottom: 1px solid rgba(0,0,0,0.1);font-size: 15px;">
          整改上报内容</h3>
        ...
        <div v-viewer class="problempiclist" style="padding-left: 16px">
          <img class="dangerpic" :src="item3" v-for="(item3, index3) in (info.rectifyImg ? info.rectifyImg : '')
              .split(',')
              .filter((item4) => item4 != '')" :key="index3" />
        </div>
        <div style="
            padding-bottom: 2px;
            font-size: 15px;
          ...
          ">
          整改日期:
          <span v-if="info.rectifyTm"
            style="margin-left: 5px; font-size: 14px; font-weight: 400">{{ info.rectifyTm.slice(0, 10) }}</span>
        </div>
      </div>
      <div style="width: calc(100% + 60px);background-color: #F3F3F3;height: 5px;margin-bottom: 20px;margin-left: -30px;"></div>
      <div class="contitle">整改执行情况<span class="sontitle"></span></div>
      <!-- 船厂待执行开始 -->
      <div class="conwrapper" v-if="
          currentRole &&
        ...
        <div style="display: flex; padding: 15px 0">
          ...
          <el-input type="textarea" :rows="5" placeholder="请输入内容" v-model="info1.rectifyReport" style="width: 277px">
          </el-input>
        </div>
        <div style="display: flex; padding: 15px 0">
          <div class="itemstyle" style="width: 104px">
          ...
          <el-upload style="margin-top: 20px" multiple="true" ref="upload" action="/general/oss/upload"
            accept="image/png,image/gif,image/jpg,image/jpeg" list-type="picture-card" :auto-upload="true"
            :before-upload="handleBeforeUpload" :on-preview="handlePictureCardPreview" :on-remove="handleRemove"
            :on-success="handSuccess">
            <i class="el-icon-plus"></i>
        ...
        <div style="display: flex; padding: 15px 0">
          ...
          <el-input type="textarea" :rows="5" placeholder="请输入内容" v-model="info.rmks" style="width: 277px"></el-input>
        </div>
        <div class="btn">
      ...
      <!--  船厂待执行结束  -->
      <!-- 检验检测待执行开始 -->
      <div class="conwrapper" v-else-if="
          currentRole &&
        ...
        <div class="item_border" v-for="(item, index) in reissueList" :key="index">
          <div
            ...
            <div>
              ...
              <div style="font-size: 15px;display: inline-block;">
                整改日期:
                <span v-if="item.rectifyTm"
                  style="margin-left: 5px; font-size: 14px; font-weight: 400">{{ item.rectifyTm.slice(0, 10) }}</span>
              </div>
            </div>
            <img style="
              position: absolute;
            ...
            }" src="~@/assets/img/personal/下拉.png" alt="" />
          </div>
          ...
          <div v-show="item.isshow">
            ...
            <div v-viewer class="problempiclist">
              <img :src="item1" class="dangerpic" alt="" v-for="(item1, index1) in (item.troubleImg
                  ? item.troubleImg
                  : ''
                )
                  .split(',')
                  .filter((item2) => item2 != '')" :key="index1" />
            </div>
            <!-- <div
          ...
          <div v-show="item.isshow && item.rectifyReport">
            ...
            <div style="padding-bottom: 2px; font-size: 15px; margin-bottom: 5px">
              执行图片：
            </div>
            <div v-viewer class="problempiclist">
              <img class="dangerpic" :src="item3" v-for="(item3, index3) in (item.rectifyImg
                  ? item.rectifyImg
                  : ''
                )
                  .split(',')
      ...
      <div style="width: calc(100% + 60px);background-color: #F3F3F3;height: 5px;margin-top: 20px;margin-bottom: 20px;margin-left: -30px;"
      v-if="
          info.state == 4 ||
          (info.state == 3 &&
            currentRole &&
      ...
      <div class="conwrapper" v-else-if="
        ...
        <div style="display: flex; padding: 15px 0">
          ...
          <el-input type="textarea" :rows="5" placeholder="请输入内容" v-model="info.closeReport" style="width: 277px">
          </el-input>
        </div>
