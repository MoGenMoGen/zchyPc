...
<div id="home" :style="{width:bWidth + 'px'}" v-show="show">
  <div class="header main">
    <el-breadcrumb class="main" separator-class="el-icon-arrow-right" :style="{width:width + 'px'}">
      ...
      <el-breadcrumb-item  :to="{path: '../learning/fishery?cid='+info.cid+'&nm='+info.cnm+'&type='+type}">{{info.cnm}}</el-breadcrumb-item>
       <el-breadcrumb-item ><p class="textBlue">课程详情</p></el-breadcrumb-item>
    </el-breadcrumb>
  </div>
  ...
  <div class="content main" :style="{width:width + 'px'}">
    <div class="left">
      <img :src="info.coverUrl" :style="{height:width*0.435*0.66 + 'px'}"/>
    </div>
    <div class="right">
      <p>{{info.nm}}</p>
      <p>{{info.summary}}</p>
      <p><span>上传日期：</span>{{info.tm}}</p>
      <!--收藏 分享-->
<!--      <div class="share">-->
...
<!--      </div>-->
      <p>
        <span v-if="info.playTm != null"><img src="@/assets/img/learning/播放时长.png"/>播放时长：{{info.playTm}}</span>
      ...
      <button v-if="info.type== 2" @click="computedPlayer()">立即观看</button>
      <button v-if="info.type == 1" @click="computedPlayer(1)">立即下载</button>
    </div>
  </div>
  <div class="body main" :style="{width:width + 'px'}">
    ...
    <div class="rt">
      <p>推荐课程</p>
      <div v-for="(v,s) in list" :key="s" @click="toPage('./learning/lessonDetail?id='+v.id+'&type='+type)">
        <img :src="v.coverUrl"/>
        <p>{{v.nm}}</p>
      </div>
    </div>
    ...
    export default {
      ...
      mounted(){
        ...
        if(!this.until.seGet('token')){
          ...
          this.show = true
          this.shareUrl = window.location.origin+'/mob/views/learning/liveDetail.html?id='+this.until.getQueryString('id')
      ...
      methods:{
        ...
        //学习资源-播放/下载计数
        async computedPlayer(type){
          await this.api.studyContPlay(this.id);
          if(type){
            window.open(this.info.url)
          }else {
            window.location.href = './learning/video2?url='+this.info.url;
          }
          //
        },
        //收藏商品
