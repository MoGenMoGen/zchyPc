  ...
  <div id="home" :style="{width:bWidth + 'px'}">
    ...
    <div class="header main" :style="{width:width + 'px'}">
      <el-breadcrumb class="board"  separator-class="el-icon-arrow-right" >
        ...
        <el-breadcrumb-item :to="{path: '../learning/fishery?cid='+cid+'&nm='+catNm+'&type='+type}">{{catNm}}</el-breadcrumb-item>
    ...
    <div class="body main" :style="{width:width + 'px'}">
        <div class="part" v-for="(item,index) in list" :key="index" :style="{height:width*0.186*1.1 + 'px'}" @click="toPage('./learning/lessonDetail?id='+item.id)">
          <img class="ad" :alt="item.nm" :src="item.coverUrl"/>
          <p class="child1">{{item.nm}}</p>
          <p class="child2">{{item.teacher}}</p>
        </div>
    </div>
  ...
  import img from '@/assets/img/测试.jpg';
    export default {
      ...
      mounted(){
        ...
        this.catNm = this.until.getQueryString('catNm')
        this.getData()
      },
      methods:{
        toSearch(){
